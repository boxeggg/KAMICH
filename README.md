...
**Cross-platform work-hours & revenue tracker for a rail-road excavator rental business — built on live telemetry, not spreadsheets.**
...
| Layer | Technology |
|---|---|
| Mobile & desktop client | .NET MAUI (C#) — single codebase, native Android app + desktop layout |
...
flowchart LR
    ...
    subgraph Backend [Spring Boot API — hosted on Railway]
        LINQO -- polled --> API[REST API]
        API <--> DB[(PostgreSQL)]
    end
...
- 📈 **Trend tracking** — aggregated worked hours across day / month / year.
